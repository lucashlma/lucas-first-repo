...
`merge` 是物理正确的那条路，但有个容易踩的坑。同一张真值 HDRI 上实测：

| 括号 | 合并后峰值 | 总能量 | 中间调相对误差（中位 / 95 分位） |
| --- | --- | --- | --- |
| EV −8 / −4 / 0 | 226 | ×0.42 | 0.40% / 0.99% |
| EV −16 / −8 / 0 | 32,854 | ×1.00 | 0.41% / 0.99% |
| 真值 | 32,910 | ×1.00 | — |

括号覆盖到的区间里，合并精度非常高（中间调误差 0.4%，这是 8 bit 量化的极限），
但**日面亮度是天空的一万倍量级**：这张图的峰值是 32,910，也就是最暗那一张的
曝光得压到 −15 EV 以下才装得下。手持包围曝光一般只到 −8 EV，日面照样是削顶的，
总能量只能恢复 42%。

所以实际操作是：括号负责天空和地面的准确亮度，日面用 `--sun` 单独补，
或者拍的时候上 ND 滤镜把曝光再压十几档。
